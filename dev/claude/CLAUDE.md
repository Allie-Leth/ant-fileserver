...
### **🚨 NEVER MODIFY LIVE CONTAINERS DIRECTLY**
**ALL changes must be made through code, YAML manifests, or configuration files:**

- ❌ **NEVER**: `kubectl exec` to modify container state
- ❌ **NEVER**: Direct API calls to running services  
- ❌ **NEVER**: Manual configuration changes in live systems
- ✅ **ALWAYS**: Update YAML manifests, Helm charts, or Kustomize configs
- ✅ **ALWAYS**: Commit changes to Git for reproducibility
- ✅ **ALWAYS**: Use GitOps workflows for deployments

**Violations of this principle break:**
- Reproducibility (changes can't be recreated)
- Auditability (no Git history of changes)
- Disaster recovery (changes lost if containers restart)
- Team collaboration (changes invisible to other developers)

### Examples:
```bash
# ❌ WRONG: Direct container modification
kubectl exec -n minio minio-0 -- mc admin user add local user pass

# ✅ CORRECT: Update YAML manifest
# Edit sealed-secret.yaml or configmap.yaml, then:
git add . && git commit -m "feat: add new MinIO user"
...
#### Environment Flow
- **main**: Production environment - only stable, tested code
- **staging**: Pre-production validation - ensure everything works before production
- **dev**: Development integration - testing and validation environment
- **feature/***: Feature development branches - isolated development work

#### Workflow
1. **Feature Development**: Create `feature/description` branch from `dev`
2. **Development Testing**: Merge feature to `dev` for integration testing
3. **Staging Validation**: Promote `dev` to `staging` for pre-production validation
4. **Production Release**: Promote `staging` to `main` for production deployment

#### GitOps Pipeline Integration
- **dev** → Deploy to development MinIO (`minio-dev` bucket)
- **staging** → Deploy to staging MinIO (`minio-staging` bucket)  
- **main** → Deploy to production MinIO (`minio` bucket)

All changes must go through the complete pipeline: feature → dev → staging → main
