...
def test_upload_with_factory_versions(svc):
    """Upload should store bootloader_version and partition_table_version."""
    payload = {
        "version": "4.0.0",
        "firmware_b64": base64.b64encode(b"firmware data").decode(),
        "release_notes": "with factory versions",
        "bootloader_version": "1.0.0",
        "partition_table_version": "2.0.0",
    }
    svc.upload_firmware("acme", "widget", payload)

    meta = svc.get_latest("acme", "widget")
    assert meta.version == "4.0.0"
    assert meta.bootloader_version == "1.0.0"
    assert meta.partition_table_version == "2.0.0"


def test_upload_without_factory_versions(svc):
    """Upload without factory versions should default to None."""
    payload = {
        "version": "5.0.0",
        "firmware_b64": base64.b64encode(b"firmware data").decode(),
    }
    ...
    assert meta.bootloader_version is None
    assert meta.partition_table_version is None
